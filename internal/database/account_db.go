...
func (a *AccountDB) FindByID(id string) (*entity.Account, error) {
	...
	stmt, err := a.DB.Prepare("SELECT a.id,a.client_id,a.balance,a.created_at,a.updated_at,c.id,c.name,c.email FROM accounts a INNER JOIN clients c ON a.client_id = c.id WHERE a.id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
